-- A reused operation must acknowledge exactly the recorded external movement.
-- Preserve existing financial rows, authorization, transaction boundaries, and grants.
...
create or replace function private.reverse_external_refund(
...
begin
  ...
  where operation_id = p_operation_id;
  if existing.id is not null then
    if existing.entry_kind is distinct from 'reversal'
      or existing.reversal_of is distinct from p_refund_record_id
      or existing.reversal_reason is distinct from btrim(p_reason)
      or existing.external_moved_at is distinct from p_external_moved_at
      or existing.recorded_by is distinct from caller
      or not exists (
        select 1 from public.payment_transactions as payment
        where payment.id = existing.transaction_id
          and private.normalize_gcash_reference(payment.reference)
            = private.normalize_gcash_reference(p_reference)
          and payment.counterparty_display_name = btrim(p_counterparty_name)
      )
    then
      raise exception 'refund_movement_retry_changed' using errcode = '40001';
    end if;
