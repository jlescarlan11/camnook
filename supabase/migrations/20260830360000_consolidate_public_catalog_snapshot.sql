create function private.get_public_catalog_snapshot()
returns jsonb
language sql
stable
security definer
...
create function api.get_public_catalog_snapshot()
returns jsonb
language sql
stable
security definer
set search_path = ''
as $$
  select private.get_public_catalog_snapshot();
$$;

revoke all on function private.get_public_catalog_snapshot()
from public, anon, authenticated;
revoke all on function api.get_public_catalog_snapshot()
from public, anon, authenticated;
grant execute on function api.get_public_catalog_snapshot()
