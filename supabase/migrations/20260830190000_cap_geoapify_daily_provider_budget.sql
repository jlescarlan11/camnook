create table private.geoapify_provider_daily_windows (
  window_started_on date primary key,
  request_count integer not null check (request_count >= 0)
);

revoke all on table private.geoapify_provider_daily_windows
...
begin
  ...
  delete from private.geoapify_provider_daily_windows
  where window_started_on < daily_window - 7;
  ...
  begin
    ...
    insert into private.geoapify_provider_daily_windows as budget (
      window_started_on, request_count
    ) values (daily_window, p_request_count)
    on conflict (window_started_on) do update
    set request_count = budget.request_count + excluded.request_count
    where budget.request_count + excluded.request_count <= 3000
    returning request_count into daily_claimed;

    if daily_claimed is null then
      raise exception 'Geoapify daily budget is exhausted' using errcode = 'P0001';
    end if;
