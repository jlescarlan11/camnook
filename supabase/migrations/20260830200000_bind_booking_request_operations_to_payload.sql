alter table private.booking_request_operations
add column request_fingerprint text;

create or replace function private.claim_booking_request_operation(
...
begin
  ...
  insert into private.booking_request_operations (
    actor_user_id, operation_id, request_fingerprint
  ) values (p_actor_user_id, p_operation_id, p_request_fingerprint)
  ...
  if existing_fingerprint is distinct from p_request_fingerprint then
    raise exception 'booking request operation payload does not match' using errcode = '22023';
  end if;
...
revoke all on function private.claim_booking_request_operation(uuid, uuid, text)
