...
export default async function ManageCameraPage({ params, searchParams }: { params: Promise<{ cameraId: string }>; searchParams: Promise<{ step?: string }> }) {
  ...
  return <div className="min-h-screen bg-stone-50 text-stone-950"><SiteHeader /><main className="mx-auto max-w-4xl px-5 py-8 sm:px-8 sm:py-12"><OwnerNav current="cameras" /><Link className="mt-6 inline-flex min-h-11 items-center font-semibold text-[#0b4f9c] underline" href="/admin/cameras">← Cameras</Link><nav aria-label="Camera setup" className="mt-5 grid grid-cols-3 gap-2 text-sm">{[["camera","1. Camera"],["availability","2. Availability"],["preview","3. Publish"]].map(([key,label]) => <Link aria-current={step === key ? "step" : undefined} className="rounded-xl border border-stone-300 bg-white px-3 py-3 text-center font-semibold aria-[current=step]:bg-stone-950 aria-[current=step]:text-white" href={`/admin/cameras/${camera.id}?step=${key}`} key={key}>{label}</Link>)}</nav>
    ...
    {step === "preview" ? (
      <section className="mt-6 rounded-xl border border-stone-200 bg-white p-6 sm:p-8">
        <h1 className="text-3xl font-semibold">Publish</h1>
        <article className="mt-6 rounded-xl border border-stone-200 bg-stone-50 p-5">
          <h2 className="text-2xl font-semibold">{camera.name}</h2>
          <details className="mt-3 text-stone-600">
            <summary className="cursor-pointer font-semibold text-[#0b4f9c]">Description</summary>
            <p className="mt-2 leading-7">{camera.description}</p>
          </details>
          <dl className="mt-4 grid gap-3 sm:grid-cols-2">
            <Value label="Daily price" value={php(camera.daily_rate)} />
            <Value label="Deposit" value={php(camera.security_deposit)} />
            <Value label="Pickup area" value={camera.handoff?.pickup_area ?? "Not set"} />
            <Value label="Photos" value={String(camera.photo_count)} />
          </dl>
        </article>
        <ul className="mt-6 space-y-2 text-sm">
          {Object.entries(ready).map(([key, value]) => (
            <li className={value ? "text-emerald-800" : "text-red-800"} key={key}>
              {value ? "✓" : "○"} {key === "meetups" ? "Meetup places" : key === "photos" ? "Photos" : key === "price" ? "Price" : key === "deposit" ? "Deposit" : "Availability"}
            </li>
          ))}
        </ul>
        {camera.status === "published" ? (
          <div className="mt-6 flex flex-wrap items-center justify-between gap-3 rounded-xl border border-emerald-200 bg-emerald-50 p-5">
            <h2 className="font-semibold text-emerald-950">Published</h2>
            <Link className="font-semibold underline" href={`/cameras/${camera.slug}`}>View listing</Link>
          </div>
        ) : Object.values(ready).every(Boolean) ? (
          <PublishCameraForm cameraId={camera.id} />
        ) : (
          <div className="mt-4 space-y-3">
            <p className="text-amber-900">Complete the remaining items before publishing.</p>
            {!ready.photos ? <Link className="block underline" href={`/admin/cameras/${camera.id}?step=camera#camera-photos`}>Add photos</Link> : null}
            {!ready.availability || !ready.meetups ? <Link className="block underline" href={`/admin/cameras/${camera.id}?step=availability`}>Set availability and meetup places</Link> : null}
            {!ready.price || !ready.deposit ? <Link className="block underline" href={`/admin/cameras/${camera.id}?step=camera`}>Edit price and deposit</Link> : null}
          </div>
        )}
      </section>
    ) : null}
