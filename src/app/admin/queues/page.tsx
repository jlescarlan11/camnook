...
export default async function AllQueuesPage() { const context = await requirePageUser("/admin/queues"); const data = await loadAdminDashboardContext(context, null); if ("forbidden" in data) redirect("/forbidden"); return <div className="min-h-screen bg-stone-50"><SiteHeader /><main className="page-shell py-8 sm:py-12"><AdminNav current="queues" /><p className="eyebrow mt-8">Owner operations</p><h1 className="page-heading mt-3">All booking queues</h1>{data.operations.status === "success" ? <OwnerOperationsPanel dashboard={data.operations.dashboard} /> : <p className="mt-6" role="alert">Queues are unavailable.</p>}</main></div>; }
