      ...
      {result.status === "error" ? <CameraLoadError query={query} slug={slug} /> : <>
        <section className="mt-5 grid gap-6 border-b border-[#d8e0ea] pb-7 sm:grid-cols-[8rem_minmax(0,1fr)] sm:items-center">
          ...
          <div className="min-w-0">
            <h1 className="text-3xl font-semibold tracking-[-.035em] sm:text-4xl">{result.camera.name}</h1>
            <p className="mt-2 text-sm text-[#58677d]">{result.camera.handoffPolicy?.cityLabel ?? "Handoff area unavailable"} · {phpFormatter.format(result.camera.dailyRate)} / day · {phpFormatter.format(result.camera.securityDeposit)} deposit</p>
