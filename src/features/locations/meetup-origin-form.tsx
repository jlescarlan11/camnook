...
export function MeetupOriginForm({ origin }: { origin: null | { areaName: string; precision: string; valid: boolean } }) {
  ...
  const areaError = saveState.status === "error" && saveState.error === "invalid";
  return (
    <div className="mt-5 space-y-4">
      ...
      <form action={saveAction} className="space-y-4">
        <PsgcAreaSelector
          errorId={areaError ? "meetup-origin-area-error" : undefined}
          invalid={areaError}
        />
        ...
        {saveState.status === "error" ? <p className="text-sm text-red-800" id={areaError ? "meetup-origin-area-error" : undefined} role="alert">{saveState.error === "invalid" ? "Choose a current barangay." : saveState.error === "provider" ? "The barangay center is unavailable. Retry without losing your selection." : "The default could not be saved. Your previous default is unchanged."}</p> : null}
