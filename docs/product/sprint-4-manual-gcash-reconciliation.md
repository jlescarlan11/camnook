...
| --- | --- | --- | --- | --- | --- |
| S4-47-1 | #47 body | During the 28 Sep–11 Oct 2026 sprint scope, show authoritative amounts and approved recipient; accept a timely owner submission with optional private proof. | `api.get_my_payment_state`, `api.submit_payment`, proof-intent lifecycle, and renter `PaymentPanel`. | `009_manual_gcash_reconciliation.sql`; payment action/data/component tests. | SATISFIED |
...
| S4-49-1 | #49 AC1 | Submission transaction rechecks auth, ownership, `TO_PAY`, current signed contract, required fields, and database time. | `private.submit_payment` plus strict Server Action input schemas. | SQL invalid actor/state/contract/config/deadline checks and action validation tests. | SATISFIED |
