...
arrays. A dashboard-specific resolution snapshot computes only the minimized
issue and cancellation slices; it does not build and discard the legacy return
or terminal-deposit queues. Supporting identity and cancellation queues preserve
earlier workflows
...
| `request_booking` | Server-only meetup-bound request creates renter-owned `FOR_REVIEW` booking, immutable meetup plan, and history; a profile-row lock caps each renter at 10 open review requests; legacy authenticated request RPCs are not executable; no availability block |
...
The repository contains forty-one forward migrations. On 13 August 2026, the four
